...
/**
 * includeToken()
 * - sets axios.defaults.baseURL to API_ENDPOINT (e.g. http://localhost:8888/api)
 * - sets withCredentials
 * - attaches Authorization header when token exists
 * - removes Authorization header when token does NOT exist (prevents empty header)
 */
function includeToken() {
  // Use the full API endpoint (includes '/api') so entity paths are appended reliably
  ...
  if (auth && auth.current && auth.current.token) {
    ...
    // remove Authorization header if no token (important to avoid sending empty header)
